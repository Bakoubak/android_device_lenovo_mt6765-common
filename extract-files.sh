...
function blob_fixup() {
    case "${1}" in
        ...
        vendor/lib*/hw/audio.primary.mt6765.so)
            ...
            "${PATCHELF}" --replace-needed "libalsautils.so" "libalsautils-v29.so" "$2"
            ;;
        ...
        vendor/lib/libMtkOmxVdecEx.so)
            ...
            ;;
        vendor/lib*/libdlbdsservice.so|vendor/lib*/soundfx/libswdap.so)
...
if [ -z "${ONLY_TARGET}" ]; then
    ...
    extract "${MY_DIR}/proprietary-firmware.txt" "${SRC}" "${KANG}" --section "${SECTION}"
fi
...
if [ -z "${ONLY_COMMON}" ] && [ -s "${MY_DIR}/../${DEVICE}/proprietary-files.txt" ]; then
    ...
    extract "${MY_DIR}/../${DEVICE}/proprietary-firmware.txt" "${SRC}" "${KANG}" --section "${SECTION}"
fi
